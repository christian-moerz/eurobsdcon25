...
MAIL_PLUGIN=https://github.com/nextcloud-releases/mail/releases/download/v5.4.0/mail-v5.4.0.tar.gz
NEXTCLOUD_ZIP=https://download.nextcloud.com/server/releases/nextcloud-31.0.9.zip
...
if [ ! -e nextcloud.zip ]; then
    fetch -o nextcloud.zip ${NEXTCLOUD_ZIP}
fi
if [ ! -e nextcloud_mail.tar.gz ]; then
    fetch -o nextcloud_mail.tar.gz ${MAIL_PLUGIN}
fi
...
if [ ! -e /ca/pki/issued/cloud.ny-central.lab.crt ]; then
    CURRENT=$(pwd)
    cd /ca
    easyrsa build-server-full cloud.ny-central.lab nopass
    cd ${CURRENT}
fi
...
if [ -e cloud.pkg-cache.tar.xz ]; then
    ssh_copy cloud.pkg-cache.tar.xz 13
fi
...
scp -o ConnectionAttempts=50 -o ConnectTimeout=3600 \
    -i .ssh/id_ecdsa lab@10.193.167.13:cloud.pkg-cache.tar.xz .

#
# Thinks to consider for real world implementations:
#
# * IP / geo blocking
# * whitelisting IPs based on nginx, pf
#
