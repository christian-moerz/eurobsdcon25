...
set +e
cat /etc/group | grep sshusers > /dev/null 2>&1
if [ "0" != "$?" ]; then
...
# Create a test user
set +e
cat /etc/passwd | grep ${TESTUSER} >/dev/null 2>&1
if [ "0" != "$?" ]; then
...
install -m 0400 /home/lab/server.key /usr/local/etc/ssl/server.key
install -m 0444 /home/lab/server.crt /usr/local/etc/ssl/server.crt
install -m 0444 -o cyrus /home/lab/server.key /usr/local/etc/ssl/cyrus.key
install -m 0444 /home/lab/ca.crt /usr/local/etc/ssl/ca.crt
install -m 0444 /home/lab/ca.crt /etc/ssl/ca.crt
...
cyrus_newuser()
{
    ...
    echo -e "D0 LOGIN cyrus ${CYRUSPASS}\nD1 CREATE user/$1\nD2 SETACL user.$1 $1 lrswipkxtea\nD3 LOGOUT\n" | openssl s_client -connect localhost:993 -crlf -CAfile /etc/ssl/ca.crt
}
...
xz /home/lab/mailsrv.pkg-cache.tar
chown lab:lab /home/lab/mailsrv.pkg-cache.tar.xz
