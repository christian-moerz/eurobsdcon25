...
MAJOR=14
MINOR=3
BASE=http://ftp.freebsd.org/pub/FreeBSD/releases/amd64/amd64/${MAJOR}.${MINOR}-RELEASE/base.txz
KERNEL=http://ftp.freebsd.org/pub/FreeBSD/releases/amd64/amd64/${MAJOR}.${MINOR}-RELEASE/kernel.txz
SRCURL=http://ftp.freebsd.org/pub/FreeBSD/releases/amd64/amd64/${MAJOR}.${MINOR}-RELEASE/src.txz
ISO=http://ftp.freebsd.org/pub/FreeBSD/releases/amd64/amd64/ISO-IMAGES/${MAJOR}.${MINOR}/FreeBSD-${MAJOR}.${MINOR}-RELEASE-amd64-disc1.iso
...
ensure_zfs()
{
    ...
    if [ "0" != "$?" ]; then
	zfs create $1
	echo "zfs destroy -r $1" >> .lab_rollback
    fi
}
...
write_installerconfig()
{
    ...
    sed -i '' 's@read TERM@@' ${ZPATH}/iso/setup/usr/libexec/bsdinstall/startbsdinstall
...
gen_media()
{
	...
	sh /usr/src/release/amd64/mkisoimages.sh -b "${MAJOR}_${MINOR}_RELEASE_AMD64_CD" ${ZPATH}/iso/$1.iso ${ZPATH}/iso/setup
