...
# 01: First script in mail server tutorial
# Sets up jail environment
...
VERS=$(freebsd-version | awk -F. '{print $1}')
if [ "${VERS}" -lt "14" ]; then
    echo "This toolset requires FreeBSD >= 14"
    exit 1
fi
...
ensure_lab_download src.txz ${SRCURL}
...
echo "rm -f /etc/jail.conf.d/${JAILNAME}.template" >> .lab_rollback
